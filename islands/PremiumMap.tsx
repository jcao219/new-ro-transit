...
export default function PremiumMap(
  ...
  useEffect(() => {
    ...
    map.current.on("load", () => {
      ...
      locations.forEach((loc) => {
        ...
        locationToMarker.current.set(
          loc,
          new mapboxgl.Marker({ element: el })
            .setLngLat(loc.coords)
            .setPopup(
              new mapboxgl.Popup({ offset: 25 }).setHTML(
                `<h3 style="color:black; font-weight:bold;">${loc.name}</h3><p style="color:gray;">${
                  loc.description || ""
                }</p>`,
              ),
            )
  ...
  const flyToLocation = (loc: Location) => {
    ...
    locationToMarker.current.get(loc)?.getElement().classList.add(
      "animate-pulse",
    );
        ...
        <div class="space-y-3">
          {filteredLocations.map((loc, idx) => (
            <div
              ...
              class={`p-3 rounded-xl border border-white/5 cursor-pointer transition-all hover:bg-white/10 ${
              ...
              <div class="flex justify-between items-start">
                ...
                <span
                  class={`w-2 h-2 rounded-full ${
                    loc.type === "restaurant" ? "bg-blue-500" : "bg-green-500"
                  }`}
                >
                </span>
              </div>
          ...
          {isFullscreen
            ? (
              ...
              >
                <path
                  fill-rule="evenodd"
                  d="M5 10a1 1 0 011-1h8a1 1 0 110 2H6a1 1 0 01-1-1zm0-4a1 1 0 011-1h8a1 1 0 110 2H6a1 1 0 01-1-1zm0 8a1 1 0 011-1h8a1 1 0 110 2H6a1 1 0 01-1-1z"
                  clip-rule="evenodd"
                />
              </svg>
            )
